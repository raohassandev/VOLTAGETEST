...
$rollbackRoot = Join-Path $DataDir "rollback"
$rollbackBackup = Join-Path $rollbackRoot (Get-Date -Format "yyyyMMdd-HHmmss")
$rollbackLog = Join-Path $LogDir "rollback.log"

function Write-RollbackLog($Message) {
  New-Item -ItemType Directory -Force -Path $LogDir | Out-Null
  Add-Content -Encoding UTF8 -Path $rollbackLog -Value "$(Get-Date -Format o) $Message"
}
...
New-Item -ItemType Directory -Force -Path $InstallDir, $DataDir, $LogDir, (Join-Path $DataDir "license"), (Join-Path $DataDir "backups"), $rollbackBackup | Out-Null

if (Test-Path $InstallDir) {
  $appBackup = Join-Path $rollbackBackup "app"
  New-Item -ItemType Directory -Force -Path $appBackup | Out-Null
  robocopy $InstallDir $appBackup /MIR | Out-Null
  if ($LASTEXITCODE -gt 7) { throw "rollback app backup failed with exit code $LASTEXITCODE" }
}
$existingEnv = Join-Path $DataDir "voltagetest.env"
if (Test-Path $existingEnv) {
  Copy-Item -Force -LiteralPath $existingEnv -Destination (Join-Path $rollbackBackup "voltagetest.env")
}

if ($SimulateFailureAfterBackup -or $env:VOLTAGETEST_SIMULATE_INSTALL_FAILURE_AFTER_BACKUP -eq "1") {
  Write-RollbackLog "Simulated failure after backup. Rollback backup: $rollbackBackup"
...
try {
  ...
  try {
    ...
    nssm set $ServiceName DisplayName "Automatrix VOLTAGETEST UMS Service"
    nssm set $ServiceName Description "Automatrix Engineering VOLTAGETEST / UMS Industrial UPS Monitoring System"
  ...
  Write-RollbackLog "Install failed: $($_.Exception.Message)"
  if ((Test-Path (Join-Path $rollbackBackup "app")) -or (Test-Path (Join-Path $rollbackBackup "voltagetest.env"))) {
    & (Join-Path $SourceRoot "release\windows-service\rollback.ps1") -BackupDir $rollbackBackup -InstallDir $InstallDir -DataDir $DataDir -ServiceName $ServiceName
  }
  throw
}
